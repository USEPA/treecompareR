...
The output of the `generate_taxonomy()` function is a list of two objects, the `phlyo` object representing the tree, and a data frame used in creating the `phylo` object. Observe the structure of these in \@ref(fig:chemont-tree). 
...
```{r clean-na-space-usgs, message = FALSE, results = 'hide', cache=TRUE}
...
```{r label-bars,  fig.align='center', fig.dim=c(6,4)}
...
```{r subtree-plots-overlap,  fig.align='center', fig.dim=c(6,4)}
display_subtree(data_1 = biosolids_classified, 
...
```{r rc-cluster-9-8, fig.align='center', fig.dim=c(12,8)}
generate_tree_cluster(tree = chemont_tree, tree_object = chemont_tree, 
