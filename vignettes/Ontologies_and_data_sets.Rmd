...
```{r}
library(data.table)
biosolids <- data.table(read.csv('../data/Chemical List BIOSOLIDS-2022-05-10.csv'))
biosolids[1:2]
names(biosolids)
```
We replace the column `PREFERRED.NAME` with `PREFERRED_NAME` and then feed this data.table to `classify_datatable()`, and then feed the output data.table from `classify_datatable()` to `classify_by_smiles()`.
...
biosolids_casrn_identifiers <- purrr::map(biosolids_casrn, get_chemical_identifiers)
biosolids_2 <- data.table(PREFERRED_NAME = unname(purrr::map(biosolids_casrn_identifiers, function(t) {
  ifelse(!is.null(t), t@meta$name, NULL)})),
                          CASRN = unname(purrr::map(biosolids_casrn_identifiers, function(t) {
                            ifelse(!is.null(t), t@meta$casrn, NULL)})),
                          INCHIKEY = unname(purrr::map(biosolids_casrn_identifiers, function(t) {
                            ifelse(!is.null(t), t@meta$inchikey, NULL)})))
