...
```{r, format-tax-nodes}
colnames(tax_nodes) <- c('Name', 'ID', 'Parent_ID')
...
ggtree::ggtree(chemont_tree) + ggtree::layout_circular()
```
...
```{r, examine-biosolids}
library(data.table)
biosolids <- data.table::data.table(biosolids)
biosolids[1:2, ]
names(biosolids)
```
...
biosolids_classified <- biosolids_class#BIOSOLIDS2021_class
```
...
```{r}
usgswater <- data.table::data.table(usgs)
usgswater[1:2,]
names(usgswater)
```
...
```{r, results = 'hide'}
usgswater_classified <- data.table::data.table(usgs_class)#data.table(USGSWATER_class)
```
