...
vignette: >
  %\VignetteIndexEntry{treecompareR package}
...
devtools::load_all()
sot_poster_subtree_mapping = list(color = c("gray50", RColorBrewer::brewer.pal(n = 3, name = "Dark2")),
                                  size = c(0.5, 1, 1, 1))
#library(kableExtra)
```
...
## ChemOnt as a `data.frame`

The ChemOnt taxonomy can be converted from JSON or OBO format to an R `data.frame` object with 4825 rows and three variables: `Name`, `ID`, and `Parent_ID`.  The taxonomy data may be imported from an OBO file using for example functions `ontologyIndex::get_ontology()` and `ontologyIndex::as.data.frame.ontology_index()`, or from a JSON file using for example `jsonlite::fromJSON()`. This `data.frame` for the ChemOnt taxonomy is provided with the package, in data object `chemont_df`. 
...
To construct a `"phylo"`-class tree object representing the ChemOnt taxonomy, 


With this data in hand, the ChemOnt `phylo`-class tree object can be constructed using function `generate_taxonomy_tree()`. The ChemOnt tree object is pre-constructed and provided with the package as `chemont_tree`; the following line of code shows how it could be constructed from `chemont_df`. If you have a different hierarchical taxonomy defined as a `data.frame` of the same format as `chemont_df`, you could use `generate_taxonomy_tree` to produce a `phylo`-class tree object for that taxonomy as well.

```{r, chemont-tree, eval = FALSE, fig.cap="chemont-tree"}
...
display_subtree(base_tree = chemont_tree,
                ...
                base_opts = list(color = "gray70")) +
 hexpand(1) 
  #ggtree:geom_hilight(node = 915)
```
...
prune_and_display_subtree(base_tree = chemont_tree,
                          ...
                          subtree_mapping = sot_poster_subtree_mapping,
                          show_tips = FALSE)
```
...
#### Get clades: all labels at a specified taxonomy level
To choose clades for pruning, you may want to list the clades of a tree at a specified level, using the function `get_all_clades()`. For this function, you specify the tree and the taxonomy level, and the output is all labels in that tree at the specified level. 

For example, the following code lists the ChemOnt superclasses (all ChemOnt labels at level 2), truncated after the first 6 items for display purposes. Superclasses are the default clade labels shown when the ChemOnt tree is plotted using `display_subtree()`.
...
In `display_subtree()`, the tree's color, size (line width), and/or linetype (solid, dashed, dotted, etc.) can be controlled using the argument `base_opts`. The default value of this argument is
...
meaning that by default, the tree will be plotted in black (`color = "black"`), with line width 0.5 (`size = 0.5`), and solid lines (`linetype = 1`). If instead we wanted to plot the tree in red, with line width 1, using dotted lines, for example, we could do so:
...
display_subtree(base_tree = oh_tree,
                ...
                subtree_mapping = c(sot_poster_subtree_mapping,
                                    'size' = c(0.1, 1, 1, 1)),
                show_tips = FALSE)
```
...
This is easiest to understand with some examples. The default value of `subtree_mapping` is the following:
```{r, eval = FALSE}
...
display_subtree(base_tree = new_tree,
                ...
                subtree_mapping = list(color = c("red", 
                                                 "black")),
                                                 #"red")),
                clade_level = 3,
                show_tips = FALSE) 
...
For example, as part of the `biosolids_classified` data set, the variable `Pct.ToxCast.Active` gives the percentage of ToxCast assays in which the chemical showed bioactivity (if the chemical has been screened in any ToxCast assays). We can visually summarize the range of percentage of active ToxCast assays for each chemical class in the Organohalogens subtree. 

We first use `display_subtree()` to create the base plot, drawing the Organohalogens subtree and highlighting the branches according to membership in the biosolids and USGS Water chemical lists. 
...
Then, we prepare a `data.frame` for use with `ggtreeExtra::geom_fruit()`. This `data.frame` should have, at minimum, one column containing the terminal labels for each entity to be plotted, and one column containing the variable to be plotted (here, `Pct.ToxCast.Active`). We can use the grammar of the `tidyverse` suite of packages to prepare the data. We want to plot chemical entities from both the `biosolids_class` and the `usgs_class` datasets.

```{r}
fruit1 <- dplyr::bind_rows(biosolids = biosolids_class, #row-bind biosolids_class and usgs_class
                              usgs = usgs_class,
                              .id = "dataset") %>% #create new column identifying dataset
  dplyr::select(dataset, #keep only relevant columns
                DTXSID, #chemical entity identifier
                terminal_label, #terminal label
                class, 
                AVERAGE.MASS) %>% #variable to plot
  dplyr::distinct() %>%  #keep only unique rows
  dplyr::filter(terminal_label %in% oh_tree$tip.label)
  
  fruit2 <- fruit1 %>%
  dplyr::group_by(terminal_label) %>%
  dplyr::summarise(dataset_all = paste(unique(dataset), #create comma-sep string listing dataset(s) for each group
                                   collapse = ", "),
                   .groups = "keep") 

  
  fruit_dat <- dplyr::left_join(fruit1, fruit2)
str(fruit_dat) #summary of resulting data.frame

```

So that we can map colors to the same list memership categories as the base tree, let's create a categorical (factor) variable from the `dataset` variable, using the same labels as in the base tree.

```{r}
fruit_dat <- fruit_dat %>%
  dplyr::mutate(list_mem = factor(dataset_all,
                                  levels = c("",
                                  "biosolids",
                                             "usgs",
                                             "biosolids, usgs"),
                                  ...
                                  "Biosolids only",
                                             "USGS Water only",
                                             "Biosolids and USGS Water")))
```


 Then, we add a visualization of the range of percentage of active ToxCast assays for each terminal label using `ggtreeExtra::geom_fruit()` with `geom = geom_boxplot`.
...
base_plot +
  ...
  ggtreeExtra::geom_fruit(data = fruit_dat,
                      ...
                      grid.params = list(),
                      pwidth = 0.4) +
  ggplot2::scale_fill_manual(name = "List presence",
                              values = c("Neither set" = "gray70",
                                  "Biosolids only" = "#66C2A5",
                                  "USGS Water only"=  "#8DA0CB",
                                  "Biosolids and USGS Water" = "#FC8D62")) +
  geom_tiplab(offset = 40, mapping = aes(color = list_presence)) +
    ggplot2::scale_color_manual(name = "List presence",
                              values = c("Neither set" = "gray70",
                                  "Biosolids only" = "#66C2A5",
                                  "USGS Water only"=  "#8DA0CB",
                                  "Biosolids and USGS Water" = "#FC8D62")) +
...
fruit_plot %>%
  add_cladelab(tree = oh_tree, clade_level = 3, clade_opts = list(offset = 50)) +
  ggtree::hexpand(0.5)
```
...
The tree visualization functions we have demonstrated thus far have focused on a qualitative visual exploration of chemical space and how each data set spreads through it. We have seen how we can examine the overlap in chemical space between data sets and how to examine more closely the subtrees representing each data set. However, we can also explore these overlaps in a more quantitative way.

The notion of similarity within ontologies and taxonomies has been an area of interest across a variety of disciplines. These aim to compare pairs of nodes in a tree with each other to gain insight on how they relate to each other. To use a family metaphor: are two nodes siblings? First cousins? First cousins once removed? 

Some of these methods involve examining just the pair of paths from the root of the tree to each of the nodes, while others may examine information more intrinsic to each node. While there is a lot to discuss and explore in the area of similarity measures, we will postpone that to the future and focus on how to use these similarity measures. The main points of similarity measures we will highlight and then demonstrate visualizations that leverage them.

## Jaccard similarity of trees
...
## Similarity measures that use information content
...
To calculate the pairwise similarity of ancestry between two lists of entities based on a specified taxonomy tree, use the function `calc_similarity()`. This function takes argument `data_1` and `data_2` (data frames of classified entities), and `tree` (a `phylo` object representing the taxonomy by which the entities are classified). It returns a matrix whose rows represent each terminal label in `data_1`, whose columns represent each terminal label in `data_2`, and whose elements represent the similarity of ancestry between each terminal label in `data_1` and each terminal label in `data_2`.

Note that the terminal labels are typically *not* the entity IDs. This means two different entities with the same terminal label will have 100% similarity.

For example, here is the similarity matrix between `biosolids_class` and `usgs_class`, considering only chemicals that are in the superclass "Organohalogen compounds" -- i.e., those in the pruned subtree `oh_tree`.

```{r}
sim_oh <- calc_similarity_data(data_1 = biosolids_class,
                data_2 = usgs_class,
                tree = oh_tree)
...
```{r}
m_rows <- biosolids_class[match(rownames(sim_oh),
                               biosolids_class$terminal_label),
                         ]
m_cols <- usgs_class[match(colnames(sim_oh),
                               usgs_class$terminal_label),
                         ]

ComplexHeatmap::Heatmap(matrix = sim_oh,
                        col = circlize::colorRamp2(seq(0, 1, len = 20),
                                                   viridis::viridis(20, option = 'C')))
```


## Heatmap visualization of simiarlity
