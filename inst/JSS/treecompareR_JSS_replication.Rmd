...
This vignette provides code to reproduce the analysis in the article "treecompareR: Tree Visualizations of Chemical Space" (Paul M. Kruse and Caroline L. Ring, 2025).

To install the development version of `treecompareR` from GitHub, execute the following code chunk.

```{r install-treecompareR, eval = FALSE}
...
#now install treecompareR from GitHub
devtools::install_github('USEPA/treecompareR')
```

If the following packages are not already installed, you will need to install them from Bioconductor: `ggtree`, `ggtreeExtra`, and `ComplexHeatmap`. To do this, evaluate the following code chunk.

```{r install-biocond-pkgs, eval = FALSE}
#install package BiocManager if not already installed
...
Each code chunk in this R Markdown document that produces a figure has another "auxiliary" code chunk immediately following it, with option `eval = FALSE`, that contains code to save that figure as a PDF. By default, those auxiliary code chunks will not be run. However, you can execute them if you would like to save the figures. 
...
In this paper, we use the `BIOSOLIDS2021` and `USGSWATER` data sets, which are available for download from the US Environmental Protection Agency's [CompTox Dashboard Chemical List](https://comptox.epa.gov/dashboard/chemical-lists) website. Note, the names of these data sets as listed on the CompTox Dashboard Chemical List are *BIOSOLIDS2021* and *USGSWATER*, respectively.
...
In a manner similar to the output of `display_subtree()` when comparing two data sets, we may wish to look at the induced subtrees of two different data sets and compare membership of labels in one subtree with the other subtree. To achieve this, we use the function `data_set_subtrees()` as we illustrate in the following example.
...
Now, the function `attach_information_content()` generates information content for an input tree, and the functions `jaccard_similarity()`, `resnik_similarity()`, `lin_similarity()`, `jiang_conrath_similarity()` all can be used to compare a pair of nodes on a given tree. If one wants to compute the similarity for all pairs of nodes in a tree, the function `similarity_matrix()` will do so for a given tree and similarity metric. The computation is lengthy for the ChemOnt tree, and so these matrices have been previously computed and stored. They are accessible as `chemont_jaccard`, `chemont_resnik_IC_SVH`, `chemont_lin_IC_SVH`, and `chemont_jiangconrath_IC_SVH`.

```{r generate-similarity-chemont, eval = FALSE}
...
To visualize the clusters through tree diagrams, we use the function `generate_tree_cluster()`, which takes in a heatmap object and a row and column cluster. It also takes in a tree corresponding to the full taxonomy and a second tree corresponding to a base tree that will be pruned. The function then returns a tree visualization (or pair of them) detailing how each data set contributes to the specific cluster under investigation.
